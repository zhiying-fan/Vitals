 ...
 */
public class CameraFeedService: NSObject {
  /**
  ...
  public func startLiveCameraSession(_ completion: @escaping(_ cameraConfiguration: CameraConfigurationStatus) -> Void) {
    sessionQueue.async {
  ...
  private func addVideoDeviceInput() -> Bool {
    ...
    do {
      ...
      if session.canAddInput(videoDeviceInput) {
        ...
        if let format = get480Format(for: camera) {
  ...
  private func get480Format(for device: AVCaptureDevice) -> AVCaptureDevice.Format? {
    let maxResolutionWidth = 480
    ...
    for format in device.formats {
      ...
      if CMFormatDescriptionGetMediaSubType(deviceFormatDescription) == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange {
        ...
        if (highestResolutionFormat == nil) || ((highestResolutionDimensions.width < maxResolutionWidth) && (candidateDimensions.width > highestResolutionDimensions.width)) {
